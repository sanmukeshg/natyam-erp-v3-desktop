...
export function formModal({
    ...
    return new Promise((resolve) => {
        ...
        // beneath it — which `.v3-shell` paints. On <body> it composited over
        ...
        (document.querySelector('.v3-shell') || document.body).append(host);
        ...
        async function submit() {
            ...
            if (Object.keys(errors).length) {
                paint();
                host.querySelector('.v3-field-error')?.closest('.v3-field')
        ...
        function paint() {
            render(host, html`
                <div class="v3-modal-scrim" data-role="scrim">
                    <div class="v3-modal v3-form-modal" role="dialog" aria-modal="true" aria-label="${title}">
                        <div class="v3-modal-head">
                            <div>
                                <h2 class="v3-modal-title">${title}</h2>
                                ${description ? html`<p class="v3-modal-sub">${description}</p>` : ''}
                            </div>
                            <button class="v3-modal-close" data-action="cancel" aria-label="Close"
                                    ${busy ? 'disabled' : ''}>
                                ${raw(icon('x', { size: 15 }))}
                            </button>
                        </div>

                        <form class="v3-modal-body" data-role="form" novalidate>
                            ${formError ? html`
                                <div class="v3-notice" data-tone="caution" data-role="form-error" role="alert">
                                    ${formError}
                                </div>
                            ` : ''}
                        ...
                        <div class="v3-modal-foot">
                            <button class="v3-ghost-btn v3-btn-md" data-action="cancel" ${busy ? 'disabled' : ''}>
                                Cancel
                            </button>
                            <button class="v3-action-btn v3-btn-md" data-action="submit" ${busy ? 'disabled' : ''}>
         ...
         * group. `.v3-field[hidden]` is already in the stylesheet.
         */
        function applyVisibility() {
            for (const f of fields) {
                if (typeof f.showIf !== 'function') continue;
                const node = host.querySelector(`.v3-field[data-field="${f.name}"]`);
        ...
        function paintCheckCounts() {
            for (const group of host.querySelectorAll('[data-checks]')) {
                ...
                const hidden = group.querySelectorAll('.v3-check[hidden]').length;
        ...
        on(host, 'input', '[data-checks-filter]', (event, target) => {
            ...
            for (const chip of group.querySelectorAll('.v3-check')) {
        ...
        on(host, 'input', 'input, textarea, select', (event) => {
            ...
            delete errors[name];
            const field = event.target.closest('.v3-field');
            if (field) {
                field.dataset.invalid = 'false';
                field.querySelector('.v3-field-error')?.remove();
            }
        });
...
function fieldMarkup(f, value, error) {
    ...
    if (f.type === 'divider') {
        return html`<p class="v3-form-divider" role="presentation">${f.label}</p>`;
    }
    ...
    return html`
        <div class="v3-field" data-invalid="${error ? 'true' : 'false'}" data-field="${f.name}">
            <label for="${id}">
...
function control(f, id, value, described) {
    ...
    if (f.type === 'checks') {
        ...
        return html`
            <div class="v3-checks-wrap">
                ${searchable ? html`
                    <input class="v3-input v3-checks-filter" type="search"
                           data-checks-filter="${f.name}"
                           placeholder="Filter ${options.length} ${f.itemNoun || 'option'}s…"
                           aria-label="Filter the list below">
                ` : ''}
                <div class="v3-checks" role="group" aria-describedby="${described}"
                     data-checks="${f.name}">
                    ${options.map((o) => html`
                        <label class="v3-check" data-label="${String(o.label).toLowerCase()}">
                            <input type="checkbox" name="${f.name}" value="${o.value}"
                                   ${chosen.has(String(o.value)) ? 'checked' : ''}>
                            <span class="v3-check-chip">${o.label}</span>
                        </label>
                    `)}
                </div>
                ${searchable ? html`
                    <span class="v3-field-help" data-checks-count="${f.name}"></span>
                ` : ''}
            </div>
        `;
    }
    ...
    if (f.type === 'textarea') {
        return html`
            <textarea class="v3-input v3-textarea" id="${id}" name="${f.name}"
    ...
    if (f.type === 'switch') {
        return html`
            <label class="v3-switch">
                <input type="checkbox" id="${id}" name="${f.name}" ${value ? 'checked' : ''}
                       aria-describedby="${described}">
                <span class="v3-switch-track"></span>
                <span class="v3-switch-label">${f.switchLabel || 'Yes'}</span>
            </label>
        `;
    }
    ...
    return raw(`
        <div class="${f.type === 'money' ? 'v3-money-wrap' : ''}">
            ${f.type === 'money' ? '<span class="v3-money-prefix">₹</span>' : ''}
            <input class="v3-input ${f.type === 'money' ? 'v3-money-input' : ''}" type="${type}"
                   ${common} ${aria} ${attrs}
...
export function confirmModal({ title, message, confirmLabel = 'Confirm', tone = 'caution' }) {
    return new Promise((resolve) => {
        ...
        render(host, html`
            <div class="v3-modal-scrim" data-role="scrim">
                <div class="v3-modal v3-form-modal" role="alertdialog" aria-modal="true" aria-label="${title}">
                    <div class="v3-modal-head">
                        <h2 class="v3-modal-title">${title}</h2>
                    </div>
